#!/usr/bin/env bash
# create_zip.sh
# Genera la estructura de la app (backend + frontend), escribe los archivos principales y empaqueta todo en calculadora_scientifica.zip
# Uso: bash create_zip.sh
set -euo pipefail

OUT_ZIP="calculadora_scientifica.zip"
TMPDIR=$(mktemp -d)
ROOT="$PWD"
WORKDIR="$TMPDIR/calculadora_scientifica"
